    ...
    <form class="book-review-form" action="/saveReview" method="post" enctype="multipart/form-data">
        ...
        <?php else : ?>
            ...
            <div class="select-section">
                <select id="bookId" name="bookId" <?= isset($reviewId) ? "disabled" : "required"?>>
                    <?php
                        foreach ($_SESSION['books'] as $book) {
                            echo "<option value=\"".$book["bookId"]."\"";
                            if (isset($_SESSION['readBookId']) && $book["bookId"] == $_SESSION['readBookId']) {
                                echo "selected";
                            }
                            echo ">".$book["name"]." by ".$book["author"]."</option>";
                        }?>
                </select>
                <?php if (!isset($_SESSION['readBookId'])): ?>
